...
class SemanticIntentResolver:
    ...
    def resolve_media_action(cls, text: str) -> Optional[Tuple[str, Optional[str]]]:
        ...
        # Müzik çalma talebi ('spotifydan Duman aç', 'bize bir Sezen Aksu çalsana')
        music_play_triggers = ["çal", "aç", "oynat", "dinlet", "başlat"]
        has_play_verb = any(k in cleaned for k in music_play_triggers)

        music_context = any(k in cleaned for k in ("spotify", "müzik", "şarkı", "parça", "albüm"))
        if has_play_verb and music_context:
            # Hedef sanatçı/şarkı çıkarımı
            cand = cleaned
            for noise in ("spotify'dan", "spotifydan", "spotifyda", "spotify", "şarkısını", "şarkısı", "parçasını", "müziğini", "bir", "şu", "açsana", "çalsana", "aç", "çal", "oynat"):
                cand = re.sub(rf"\b{re.escape(noise)}\b", "", cand)
            cand = cand.strip()
            if cand and len(cand) > 1 and cand not in ("müzik", "şarkı"):
                return "play_target", cand.title()
