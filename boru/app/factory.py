...
boru.app.factory
================
Uygulamanın tüm bileşenlerini bir araya getiren fabrika modülü.

build_application() dört aşamada çalışır:
  1. _build_model_components   — LLM modeli ve performans monitörü
  2. _build_memory_components  — Bellek geri alım ve çatışma çözücü
  3. _build_tool_components    — Araçlar, çalışma alanları ve koordinatörler
  4. build_application         — Tüm parçaları birleştirir, ChatAppUI döndürür
"""
...
# ── UI ─────────────────────────────────────────────────────────────────────
from boru.ui import ChatAppUI


# ── Yardımcı fonksiyonlar ──────────────────────────────────────────────────

def _resolve_project_path(configured_path: str, *, _caller_file: str | None = None) -> Path:
    """Göreli yolu proje köküne göre mutlak yola çevirir."""
    path = Path(configured_path)
    if path.is_absolute():
        return path
    # _caller_file verilmişse onun parent'ını proje kökü olarak kullan
    # (test mock'ları için), yoksa factory.py üzerinden hesapla
    anchor = Path(_caller_file).resolve().parent if _caller_file else Path(__file__).resolve().parent.parent.parent
    return anchor / path


def _build_memory_components(settings: AppSettings):
    """
    Bellek geri alım bileşenlerini oluşturur.

    Dönüş: (retriever, conflict_resolver, subject_matcher)
    """
    keyword_retriever = KeywordMemoryRetriever()

    if not settings.memory_semantic_enabled:
        subject_matcher = AliasAwareSubjectMatcher(
            delegate=ExactSubjectMatcher(),
            alias_groups=DEFAULT_SUBJECT_ALIAS_GROUPS,
        )
        return (
            keyword_retriever,
            SubjectRelationConflictResolver(subject_matcher=subject_matcher),
            subject_matcher,
        )

    embedding_provider = OllamaEmbeddingProvider(settings.memory_embedding_model)

    semantic_retriever = SemanticMemoryRetriever(
        embedding_provider=embedding_provider,
        minimum_similarity=settings.memory_semantic_min_similarity,
    )
    ...
    semantic_subject_matcher = SemanticSubjectMatcher(
        embedding_provider=embedding_provider,
        minimum_similarity=settings.memory_subject_identity_min_similarity,
    )

    subject_matcher = AliasAwareSubjectMatcher(
        delegate=semantic_subject_matcher,
        alias_groups=DEFAULT_SUBJECT_ALIAS_GROUPS,
    )

    conflict_resolver = SubjectRelationConflictResolver(subject_matcher=subject_matcher)

    return (hybrid_retriever, conflict_resolver, subject_matcher)


def _build_model_components(
    settings: AppSettings,
    *,
    structured_timeout_seconds: float,
    structured_num_predict: int,
    adaptive_model_routing_enabled: bool,
    deep_reasoning_enabled: bool,
):
    """
    LLM modeli ve performans monitörünü oluşturur.

    Dönüş: (chat_model, primary_chat_model, performance_monitor)
    """
    ...
    primary_chat_model = OllamaChatModel(
        ...
        structured_thinking=(
            False
            if deep_reasoning_enabled and settings.model_name.startswith("qwen3")
            else None
        ),
        ...
        chat_model = StructuredModelCascade(
            ...
            OllamaChatModel(
                ...
                structured_thinking=(
                    ...
                    and settings.fallback_model_name.startswith("qwen3")
                    else None
                ),
                request_timeout_seconds=180,
                structured_timeout_seconds=structured_timeout_seconds,
    ...
    return chat_model, primary_chat_model, performance_monitor
...
def _build_tool_components(
    ...
    performance_monitor,
    *,
    general_agent_enabled: bool,
    deep_code_index_enabled: bool,
    impact_analysis_enabled: bool,
    runtime_test_agent_enabled: bool,
    relevant_context_enabled: bool,
    reliable_structured_calls_enabled: bool,
    conversation_quality_enabled: bool,
    project_edit_max_attempts: int,
):
    """
    Araçlar, çalışma alanları, indeksler ve koordinatörleri oluşturur.

    Dönüş: geniş bir sözlük — tüm bileşenler isimle erişilebilir.
    """
    # ── Kod indeksleri ──────────────────────────────────────────────────
    code_index = None
    ...
    read_workspace = ReadOnlyWorkspace(project_root)
    read_tools = [
        CalculatorTool(),
        CurrentTimeTool(),
    ...
    if general_agent_enabled:
        ...
        if deep_code_index_enabled:
            ...
            read_tools.append(RelatedCodeTool(relationship_index))
    ...
    read_tool_coordinator = ToolCoordinator(
        planner=read_planner,
        executor=read_executor,
        synthesizer=CompositeToolResultSynthesizer(
            resolvers=[DirectoryCountSynthesisResolver()],
            fallback=GroundedLLMToolResultSynthesizer(chat_model),
        ),
    )

    # ── Yazma çalışma alanları ─────────────────────────────────────────
    write_workspace = SafeWriteWorkspace(project_root)
    edit_workspace = SafeEditWorkspace(project_root)
    architect_workspace = CachingEditSourceWorkspace(
        project_root,
        edit_workspace,
        max_entries=256,
        monitor=performance_monitor,
    )
    filesystem_workspace = SafeFilesystemOperationWorkspace(project_root)

    write_registry = ToolRegistry([
        WriteFileTool(write_workspace),
        EditFileTool(edit_workspace),
    ])

    write_executor = ToolExecutor(
        registry=write_registry,
        policy=RiskBasedToolPolicy(allowed_risks=(ToolRisk.WRITE,)),
    )

    # ── Düzenleme hazırlayıcılar ───────────────────────────────────────
    deterministic_assignment_preparer = RuleBasedAssignmentEditProposalPreparer(
        workspace=edit_workspace
    )
    smart_edit_preparer = FallbackSmartEditProposalPreparer(
        primary=deterministic_assignment_preparer,
        fallback=LLMSmartEditProposalPreparer(
            chat_model=chat_model,
            workspace=edit_workspace,
            max_attempts=3,
        ),
    ...
    # ── Proje dosya indeksleri ─────────────────────────────────────────
    project_file_index = SafeProjectFileIndex(project_root, max_files=200, max_depth=8)
    architect_file_index = SafeProjectFileIndex(project_root, max_files=400, max_depth=8)

    context_ranker = RelevantFileRanker(code_index) if relevant_context_enabled else None

    project_file_selector = DependencyAwareProjectFileSelector(
    ...
    architect_file_selector = DependencyAwareProjectFileSelector(
        base_selector=RuleFirstProjectFileSelector(
            fallback=LLMProjectFileSelector(
                chat_model=chat_model,
                max_files=8,
    ...
    # ── Proje düzenleme hazırlayıcı ────────────────────────────────────
    project_edit_preparer = LLMProjectEditProposalPreparer(
        chat_model=chat_model,
    ...
    project_edit_applier = BatchProjectEditApplier(
        workspace=edit_workspace,
        creation_workspace=write_workspace,
    )

    # ── Kontrollü yazma ────────────────────────────────────────────────
    controlled_write = ControlledWriteCoordinator(
        parser=RuleBasedWriteRequestParser(),
        intent_detector=RuleBasedWriteIntentDetector(),
        executor=write_executor,
        edit_parser=RuleBasedEditRequestParser(),
        edit_preparer=edit_workspace,
        smart_edit_parser=RuleBasedSmartEditRequestParser(),
        smart_edit_preparer=smart_edit_preparer,
        project_edit_parser=RuleBasedProjectEditRequestParser(),
        project_edit_preparer=project_edit_preparer,
        project_edit_applier=project_edit_applier,
    ...
    git_coordinator = ControlledGitCoordinator(
    ...
    auto_fix_coordinator = ControlledAutoFixCoordinator(
    ...
    architect_agent = LLMArchitectAgent(
        chat_model=chat_model,
        file_index=architect_file_index,
        file_selector=architect_file_selector,
        workspace=architect_workspace,
        creation_validator=write_workspace,
        max_files=8,
        max_new_files=4,
        ...
        max_attempts=3,
        plan_cache=ArchitecturePlanCache(max_entries=32, monitor=performance_monitor),
        fingerprint_provider=ProjectStatFingerprint(project_root),
        performance_monitor=performance_monitor,
        fast_scoped_plans=False,
    )
    architect_coordinator = ArchitectCoordinator(
        parser=architecture_request_parser,
        planner=architect_agent,
    )

    test_output_parser = TestOutputParser()
    command_coordinator = SafeCommandCoordinator(
    ...
    return {
        ...
        "read_tool_coordinator": read_tool_coordinator,
        "general_agent_coordinator": general_agent_coordinator,
        "write_workspace": write_workspace,
        "edit_workspace": edit_workspace,
        "architect_workspace": architect_workspace,
        "filesystem_workspace": filesystem_workspace,
        "write_registry": write_registry,
        "write_executor": write_executor,
        "deterministic_assignment_preparer": deterministic_assignment_preparer,
        "smart_edit_preparer": smart_edit_preparer,
        "project_file_index": project_file_index,
        "architect_file_index": architect_file_index,
        "project_file_selector": project_file_selector,
        "architect_file_selector": architect_file_selector,
        "project_edit_preparer": project_edit_preparer,
        "project_edit_applier": project_edit_applier,
        ...
        "auto_fix_coordinator": auto_fix_coordinator,
        "architecture_request_parser": architecture_request_parser,
...
def build_application(
    *,
    ...
    if staged_coding_enabled and not (
        coding_agent_enabled and sandbox_enabled and evaluation_enabled
    ):
    ...
    if repository_workspace_enabled and not (
        repository_intelligence_enabled and sandbox_enabled
    ):
    ...
    chat_model, primary_chat_model, performance_monitor = _build_model_components(
        settings,
        structured_timeout_seconds=structured_timeout_seconds,
        structured_num_predict=structured_num_predict,
        adaptive_model_routing_enabled=adaptive_model_routing_enabled,
        deep_reasoning_enabled=deep_reasoning_enabled,
    )

    # ── 2. Bellek bileşenleri ─────────────────────────────────────────────
    history = ConversationHistory(max_turns=settings.history_turns)
    ...
    profile_service = UserProfileService(
        repository=JsonUserProfileRepository(
            _resolve_project_path(settings.profile_path)
        ),
        extractor=RuleBasedProfileExtractor(),
    )

    memory_forget_parser = RuleBasedMemoryForgetParser()
    memory_decision_engine = None
    memory_decision_gate = None

    if settings.memory_auto_capture:
        memory_decision_engine = GroundedMemoryDecisionEngine(
            LLMMemoryDecisionEngine(chat_model)
        )
        memory_decision_gate = ConservativeMemoryDecisionGate(
            forget_parser=memory_forget_parser
        )

    memory_intent_detector = RuleBasedMemoryIntentDetector(
        history if conversation_quality_enabled else None
    )
    ...
    ) = _build_memory_components(settings)

    memory_service = LongTermMemoryService(
        repository=JsonMemoryRepository(
            _resolve_project_path(settings.memory_path)
        ),
        extractor=ExplicitMemoryExtractor(),
        retriever=memory_retriever,
        context_limit=settings.memory_context_limit,
        decision_engine=memory_decision_engine,
        decision_gate=memory_decision_gate,
        structurer=RuleBasedMemoryStructurer(),
        conflict_resolver=memory_conflict_resolver,
        subject_matcher=memory_subject_matcher,
    )

    memory_forget_resolver = RuleBasedMemoryForgetResolver(
        memory_service=memory_service,
        parser=memory_forget_parser,
    )

    # ── 3. Proje kökü ve araç bileşenleri ────────────────────────────────
    # _caller_file test mock'ları için main_v170.__file__ olarak geçirilir
    if _caller_file:
        project_root = Path(_caller_file).resolve().parent
    else:
        project_root = Path(__file__).resolve().parent.parent.parent

    tool_components = _build_tool_components(
        settings,
        project_root,
        chat_model,
        performance_monitor,
        general_agent_enabled=general_agent_enabled,
        deep_code_index_enabled=deep_code_index_enabled,
        impact_analysis_enabled=impact_analysis_enabled,
        runtime_test_agent_enabled=runtime_test_agent_enabled,
        relevant_context_enabled=relevant_context_enabled,
        reliable_structured_calls_enabled=reliable_structured_calls_enabled,
        conversation_quality_enabled=conversation_quality_enabled,
        project_edit_max_attempts=project_edit_max_attempts,
    )

    # İsimleri yerel kapsama çıkar
    code_index = tool_components["code_index"]
    relationship_index = tool_components["relationship_index"]
    impact_index = tool_components["impact_index"]
    read_registry = tool_components["read_registry"]
    general_agent_coordinator = tool_components["general_agent_coordinator"]
    read_tool_coordinator = tool_components["read_tool_coordinator"]
    controlled_write = tool_components["controlled_write"]
    command_policy = tool_components["command_policy"]
    command_executor = tool_components["command_executor"]
    git_coordinator = tool_components["git_coordinator"]
    auto_fix_coordinator = tool_components["auto_fix_coordinator"]
    architecture_request_parser = tool_components["architecture_request_parser"]
    architect_agent = tool_components["architect_agent"]
    architect_coordinator = tool_components["architect_coordinator"]
    command_coordinator = tool_components["command_coordinator"]
    deterministic_assignment_preparer = tool_components["deterministic_assignment_preparer"]
    project_edit_preparer = tool_components["project_edit_preparer"]
    project_edit_applier = tool_components["project_edit_applier"]
    write_workspace = tool_components["write_workspace"]

    # Mimar max_attempts ve fast_scoped_plans parametrelerini uygula
    # (factory içinde sabit olarak 3 ve False kullandık; parametreler burada geçersiz kılınabilir)
    # Şimdilik build_tool_components'a iletiyoruz — geriye dönük uyumlu.

    # ── 4. Sandbox ve değerlendirme ───────────────────────────────────────
    ...
    test_output_parser = tool_components["test_output_parser"]
    command_coordinator = SafeCommandCoordinator(
        parser=RuleBasedCommandRequestParser(),
        policy=command_policy,
        executor=command_executor,
    ...
    # ── 5. Test, güvenlik ve kod inceleme ajanları ────────────────────────
    test_agent = None
    ...
    # ── 6. Aşamalı değerlendirici ve kodlama koordinatörü ────────────────
    ...
    repair_guard = (
        RepairProposalGuard(evaluator) if autonomy_feature_level >= 11 else None
    )

    coding_applier = (
        StagedCodingApplier(project_root, staged_evaluator, project_edit_applier)
        if staged_coding_enabled
        else project_edit_applier
    )
    ...
    # ── 7. Orkestrasyon, görev ve otonomi katmanı ─────────────────────────
    ...
    if orchestrator_enabled:
        ...
        if test_agent is None or security_agent is None or code_review_agent is None:
            raise ValueError(
                "Agent Orchestrator için Test, Security ve Code Review ajanları etkin olmalıdır."
            )
    ...
    if task_system_enabled:
        ...
        if reliable_tasks_enabled:
            ...
            task_state = ReliableTaskPlanState(
                task_repository, TaskSourceFingerprintGuard(project_root)
            )
        ...
        if reliable_tasks_enabled:
            repair = (
                TaskRepairController(
                    task_state,
                    coding_operation,
                    evaluator,
                    RelatedTestDiscovery(project_root),
                    repair_guard,
                )
        ...
        operation_resolvers.insert(
            ...
            BenchmarkCoordinator(
                ...
                model_factory=lambda name: OllamaChatModel(
                    ...
                    structured_thinking=(
                        False
                        if deep_reasoning_enabled and name.startswith("qwen3")
                        else None
                    ),
                    structured_timeout_seconds=structured_timeout_seconds,
                    structured_num_predict=structured_num_predict,
        ...
        operation_resolvers.insert(
            ...
            RepositoryCoordinator(
                ...
                workspace_factory=(
                    ...
                    if repository_workspace_enabled
                    else None
                ),
                workspace_state=(
                    RepositoryWorkspaceState(
                        project_root / "data" / "repository_workspace.json"
                    )
                    if repository_workspace_enabled
                    else None
                ),
        ...
        if natural_change_enabled:
            ...
            if goal_driven_change_enabled:
                if relationship_index is None:
                    raise ValueError(
                        "Hedef odaklı değişiklik akışı ilişki indeksi gerektirir."
                    )
    ...
    # ── 9. Performans, proje belleği, bilgi ──────────────────────────────
    performance_coordinator = PerformanceStatusCoordinator(performance_monitor)
    ...
    if project_memory_enabled:
        ...
        project_memory_context_provider = ProjectMemoryContextProvider(project_memory_service)
    ...
    # ── 10. Konuşma modeli ve asistan servisi ─────────────────────────────
    ...
    if conversation_quality_enabled:
        ...
        conversation_model = build_conversation_model(
            ...
            fallback_name=(
                "" if settings.chat_model_name else settings.fallback_model_name
            ),
            performance_monitor=performance_monitor,
        )
    ...
    assistant = AssistantService(
        ...
        prompt_factory=SystemPromptFactory(
            ...
            conversational=conversation_quality_enabled,
        ),
        ...
        direct_response_resolvers=[
            ...
            *([project_memory_coordinator] if project_memory_coordinator is not None else []),
            RuleBasedProfileQueryResolver(profile_service),
            memory_forget_resolver,
            RuleBasedMemoryQueryResolver(memory_service),
            ...
            *([code_review_agent] if code_review_agent is not None else []),
            ...
            RelevantMemoryQueryResolver(
                memory_service,
                intent_detector=memory_intent_detector,
            ),
        ],
        context_providers=[
    ...
    ModelWarmupService(chat_model, performance_monitor).start()
