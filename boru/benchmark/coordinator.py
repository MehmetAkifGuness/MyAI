...
class BenchmarkCoordinator:
    ...
    _USAGE = (
        ...
        "'benchmark durumu'. İsterseniz güvenli 'python -B -m boru.benchmark "
    ...
    def _run(self, request: BenchmarkRequest) -> None:
        try:
            ...
            report = self._runner_factory(request.repair_attempts).run(
                models, cases, repeats=request.repeats, budget_seconds=request.budget_seconds
            )
    ...
    def _status_detail(self) -> str:
        return f"Geçen süre: {max(0.0, monotonic() - self._started):.1f} sn"
    ...
    def _parse_native(cls, text: str) -> BenchmarkRequest:
        ...
        return cls._validated(models=models, **values)
    ...
    def _parse_flags(cls, parts: list[str]) -> BenchmarkRequest:
        ...
        while index < len(parts):
            ...
            if flag not in {"--model", "--case", *names} or index + 1 >= len(parts):
            ...
            index += 2
        return cls._validated(models=tuple(models), case_ids=tuple(cases), **values)

    @classmethod
    def _validated(cls, *, models, case_ids=(), limit, repeats, budget_seconds, repair_attempts):
        ...
        return BenchmarkRequest(models, tuple(dict.fromkeys(case_ids)), limit, repeats,
