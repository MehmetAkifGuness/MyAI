...
def build_application(
    ...
    tool_workspace = ReadOnlyWorkspace(
    ...
    tool_planner = FallbackToolPlanner(
    ...
    tool_coordinator = ToolCoordinator(
        ...
        synthesizer=CompositeToolResultSynthesizer(
            ...
            fallback=GroundedLLMToolResultSynthesizer(
                chat_model
            ),
        ),
    )
    ...
    return ChatAppUI(
        assistant=assistant,
        title="Börü V0.10.5.1",
        startup_message=(
            "Börü V0.10.5.1 hazır. "
            "Hafıza ve persistence altyapısına ek olarak güvenli "
            "Tool Core, read-only filesystem, grounded synthesis ve "
            "rule-first doğal dil planning ve fail-closed tool planning aktif."
        ),
    )
